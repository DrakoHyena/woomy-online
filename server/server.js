 ...
 */
    ...
    (async () => {
        ...
        function parseMockup(e, out, seenIndexes) {
            if (seenIndexes.has(e.index)) {
                return;
            }
            ...
            for (let i = 0; i < e.upgrades.length; i++) {
                ...
                out.push(e.upgrades[i].index);
            }
            out.push(e.guns.length);
            ...
            for (let i = 0; i < e.turrets.length; i++) {
                const turret = e.turrets[i]
                out.push(turret.index);
                out.push(turret.bound.size);
                out.push(turret.bound.offset);
                out.push(turret.bound.direction);
                out.push(turret.bound.layer);
                out.push(turret.bound.angle);
            }
            for (let i = 0; i < e.turrets.length; i++) {
                parseMockup(e.turrets[i], out, seenIndexes);
            }
        }
                ...
                async incoming(message) {
                    ...
                    switch (index) {
                        ...
                        case clientPackets.requestEntityMockups:
                            if (m.length < 1) {
                                this.error("entity image info request", "Empty entity image info request", true);
                                return 1;
                            }
                            ...
                            for (let i = 0; i < m.length; i++) {
                                const indexNum = m[i];
                                const classString = exportNames[indexNum];
                                if (!classString) {
                                    return;
                                }
                                ...
                                try {
                                    ...
                                    entity.minimalDefine(Class[classString]);
                                    parseMockup(entity, out, seenIndexes);
                                } catch (err) {
                            ...
                            this.talk(serverPackets.mockupRequest, out)
                            break;
                        case clientPackets.requestLiveEntityInfo:
                            if (m.length < 1) {
                                this.error("entity info request", "Empty entity info request", true);
                                return 1;
                            }
                            // Accept multiple entity IDs in a single request
                            this.requestedFullContextEntityIds.clear();
                            for (let i = 0; i < m.length; i++) {
        ...
        const gameLoop = (() => {
            ...
            const entitiesLiveLoop = my => {
                ...
                if (my.death()) {
                    ...
                    my.friction();
                    if (my.bond !== null) {
                        function tickTurrets(ent) {
                            for (let turret of ent.turrets) {
                                turret.life();
                                tickTurrets(turret);
                            }
                        }
