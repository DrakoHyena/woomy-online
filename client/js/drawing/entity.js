...
// Returns true if it handled stroke/fill itself (for Path2D cases), false otherwise
function drawShapeGeometry(context, shape, radius, options = {}) {
	const { widthHeightRatio, dip: customDip, arcStart = 0, arcLen = 1, ring } = options;

	if (shape === -4000) return { handled: true };

	// Bitmaps - draw relative to origin
	if (shape instanceof ImageBitmap) {
		context.drawImage(shape, -radius * shape.p1, -radius * shape.p2, radius * shape.p3, radius * shape.p4);
		return { handled: true };
	}

	// Path2Ds
	if (shape instanceof Path2D) {
		let radiusDiv = shape.path2dDiv || 1;
		let scaledRadius = radius / radiusDiv;
		context.save();
		context.scale(scaledRadius, scaledRadius);
		context.lineWidth /= scaledRadius;
	...
	if (Array.isArray(shape) || (shape && typeof shape === 'object' && !(shape instanceof Path2D))) {
		// SVG path string format: [pathString, radiusDiv]
		if (!shape[2] && typeof shape[0] === 'string') {
			let radiusDiv = shape[1] || 1;
			let path = new Path2D(shape[0]);
			let scaledRadius = radius / radiusDiv;
			context.save();
			context.scale(scaledRadius, scaledRadius);
			context.lineWidth /= scaledRadius;
		...
		// Dot-to-Dot format: [[x,y], [x,y], ...]
		context.beginPath();
		for (let [sx, sy] of shape) {
			context.lineTo(radius * sx, radius * sy);
		}
		context.closePath();
	...
	if (shape < 0) {
		let dip = customDip !== undefined ? customDip : (1 - 6 / shape / shape);
		let sides = -shape;
		...
		for (let i = 0; i < sides; i++) {
			...
			context.quadraticCurveTo(
				radius * dip * Math.cos(htheta),
				radius * dip * Math.sin(htheta),
	...
	// Circle
	if (shape === 0) {
		let arcEnd = 2 * Math.PI * arcLen + arcStart;
		context.arc(0, 0, radius, arcStart, arcEnd, false);
		if (ring !== undefined) {
			context.arc(0, 0, radius * ring, arcEnd, arcStart, true);
		}
		return { handled: false };
	}
	...
	if (shape > 0 && shape < 102) {
		if (shape === 4 && widthHeightRatio != null && (widthHeightRatio[0] !== 1 && widthHeightRatio[1] !== 1)) {
			for (let [rx, ry] of [
				...
				context.lineTo(radius * rx + (rx - 1) * 1.1, radius + (ry - 1) * 1.1);
			}
		} else {
			for (let i = 0; i < shape; i++) {
				let theta = i / shape * 2 * Math.PI;
	...
	if (shape === 102) {
		for (let [scale, theta] of [
			...
			[1, 1.2 * Math.PI],
			[1, 1.6 * Math.PI]
		]) context.lineTo(radius * scale * Math.cos(theta), radius * scale * Math.sin(theta));
		return { handled: false };
	}
	...
	if (shape === 103) { // Aquamarine Body Type
		for (let i = 0; i < 360; i++) {
			...
			if (i === 135) {
				...
				px = radius * Math.cos(((((i + 90) / 360) * 2) * Math.PI));
				py = radius * Math.sin(((((i + 90) / 360) * 2) * Math.PI));
			}
	...
	if (shape === 104) { // Star
		...
		for (let i = 0; i < 6; i++) {
			let theta = (i + 1) / 6 * 2 * Math.PI,
				...
				cx = radius * dip * Math.cos(htheta),
				cy = radius * dip * Math.sin(htheta),
				px = radius * Math.cos(theta),
				py = radius * Math.sin(theta);
	...
	if (shape === 105) { // Nautica shell shape
		for (let i = 0; i < 16; i++) {
			let theta = (i / 16) * 2 * Math.PI,
				px = radius * Math.cos(theta + .4),
				py = radius * Math.sin(theta + .4);
			if (i === 1 || i === 5 || i === 9 || i === 13) {
				px = 0;
				py = 0;
			}
	...
	if (shape === 108) { // Tri-Blade
		for (let i = 0; i < 12; i++) {
			let theta = (i / 12) * 2 * Math.PI,
				...
				py = radius * Math.sin(theta);
			if (i === 2 || i === 10 || i === 6) {
				px = (radius / 2) * Math.cos(theta);
				py = (radius / 2) * Math.sin(theta);
			}
			if (i === 0 || i === 4 || i === 8) {
				px = (radius * 1.25) * Math.cos(theta);
				py = (radius * 1.25) * Math.sin(theta);
			}
			context.lineTo(px, py);
		}
		return { handled: false };
	...
	if (shape === 112) { // Destroyer
		for (let [scale, theta] of [
			...
			[-1.16, 1.047],
			[-1, Math.PI / 2]
		]) context.lineTo(radius * scale * Math.cos((theta - .025)), radius * scale * Math.sin((theta - .025)));
		return { handled: false };
	}
	...
	if (shape === 116) { // Rodrigo's Destroyer Ship
		for (let [scale, theta] of [
			...
			[1.1, -0.335],
			[1.5, -0.14]
		]) context.lineTo(radius * (scale * 1.5) * Math.cos((theta - .0261)), radius * (scale * 1.5) * Math.sin((theta - .0261)));
		return { handled: false };
	}
	...
	if (shape === 117) { // Frigate
		for (let [scale, theta] of [
			...
			[-1, .82],
			[0.95, -0.578]
		]) context.lineTo(radius * (scale * .9) * Math.cos(theta), radius * (scale * .9) * Math.sin(theta));
		return { handled: false };
	}
	...
	if (shape === 118) { // Blue Runner
		for (let [scale, theta] of [
			...
			[-0.35, 1.361],
			[0.625, -0.698]
		]) context.lineTo(radius * scale * Math.cos((theta - .0261)), radius * scale * Math.sin((theta - .0261)));
		return { handled: false };
	}
	...
	if (shape === 122) {
		for (let [scale, theta] of [
			...
			[1.315, -0.331613],
			[1, 0]
		]) context.lineTo(radius * scale * Math.cos(theta + 1.5447), radius * scale * Math.sin(theta + 1.5447));
		return { handled: false };
	}
	...
	if (shape === 124) { // Ring
		...
		context.arc(0, 0, radius / 1.15, 0, 2 * Math.PI, false);
		return { handled: false };
	}

	if (shape === 125) { // Caravan
		let borderRadius = 0.4,
	...
	if (shape === 126) { // Cool When You Spin Shape
		let borderRadius = .4,
			centerAway = radius * (1 - borderRadius),
			scaleRadius = radius * borderRadius;
		context.arc(centerAway, centerAway, scaleRadius, 0, .5 * Math.PI);
	...
	if (shape === 128) { // Triangle (Rotated)
		...
		for (let i = 0; i < realShape; i++) {
			let theta = i / realShape * 2 * Math.PI,
				px = radius * 1.5 * Math.cos(theta + 45),
				py = radius * 1.5 * Math.sin(theta + 45);
			context.lineTo(px, py);
		}
		return { handled: false };
	}
	...
	if (shape === 130) { // exports.genericEntity
		let realShape = -11,
	...
	if (shape === 131) { // Minesweeper Ring
		context.arc(0, 0, radius, 0, 2 * Math.PI, true);
		context.arc(0, 0, radius / 1.05, 0, 2 * Math.PI, false);
		return { handled: false };
	}

	if (shape === 132) { // Glass Smasher Body
		context.arc(0, 0, radius, 0, 2 * Math.PI, true);
		context.arc(0, 0, radius / 1.5, 0, 2 * Math.PI, false);
		return { handled: false };
	}
	...
	if (shape === 133) { // Hendecagon (Rotated)
		for (let i = 0; i < 11; i++) {
			let theta = i / 11 * 2 * Math.PI,
				px = radius * 1.5 * Math.cos((180 / 11) + theta + 1.635),
				py = radius * 1.5 * Math.sin((180 / 11) + theta + 1.635);
			context.lineTo(px, py);
		}
		return { handled: false };
	}
	...
	if (shape === 134) { // Square (Rotated)
		...
		for (let i = 0; i < realShape; i++) {
			let theta = i / realShape * 2 * Math.PI,
				px = radius * 1.5 * Math.cos((180 / 4) + theta + .52),
				py = radius * 1.5 * Math.sin((180 / 4) + theta + .52);
			context.lineTo(px, py);
		}
		return { handled: false };
	}
	...
	if (shape === 135) { // Hexagon (Rotated)
		for (let i = 0; i < 6; i++) {
			let theta = i / 6 * 2 * Math.PI,
				px = radius * 1.1 * Math.cos((180 / 6) + theta + .385),
				py = radius * 1.1 * Math.sin((180 / 6) + theta + .385);
			context.lineTo(px, py);
		}
		return { handled: false };
	}
	...
	if (shape === 136) { // Revolutionist
		context.arc(0, 0, radius, 0, 2 * Math.PI, true);
		context.arc(0, 0, radius * .999999, 0, 2 * Math.PI, false);
		return { handled: false };
	}
	...
	if (shape === 140) {
		context.arc(0, 0, radius * 1.45, 0, 2 * Math.PI);
		return { handled: false };
	}
	...
	if (shape === 142) { // Boss Shape
		for (let [scale, theta] of [
			...
			[1, 270],
			[1, 288]
		]) context.lineTo(radius * scale * Math.cos((theta * Math.PI) / 180), radius * scale * Math.sin((theta * Math.PI) / 180));
		return { handled: false };
	}

	if (shape === 143) { // Boss Shape 2
		for (let [scale, theta] of [
			[1, 0],
			[0.905, 5],
			[0.915, 10],
	...
	if (shape === 145) { // Aquamarine Body Type
		for (let i = 0; i < 360; i++) {
			let theta = (i / 360) * 2 * Math.PI,
				px = radius * Math.cos(theta),
				py = radius * Math.sin(theta);
			if (i === 180) {
				...
				px = radius * Math.cos(((((i + 180) / 360) * 2) * Math.PI));
				py = radius * Math.sin(((((i + 180) / 360) * 2) * Math.PI));
			}
			context.lineTo(px, py);
		}
		return { handled: false };
	...
	if (shape === 146) { // Ring
		context.arc(0, 0, radius, 0, 2 * Math.PI, true);
		context.arc(0, 0, radius * 1.1, 0, 2 * Math.PI, false);
		return { handled: false };
	}
	...
	if (shape === 148) { // Golden Rectangle rot
		for (let [scale, theta] of [
			[1.2, .541 + Math.PI],
			[-1.2, -0.541 + Math.PI],
			[-1.2, .541 + Math.PI],
	...
	if (shape === 149) { // Semicircle
		context.arc(0, 0, radius, Math.PI / 2, 3 * Math.PI / 2);
		return { handled: false };
	}

	if (shape === 150) { // Explosive Crusher
		for (let [scale, theta] of [
			[1, 0],
			[1, 1.11],
			[-1.1, -.965],
	...
	if (shape === 157) { // Injector
		context.arc(0, 0, radius, 0, 2 * Math.PI, true);
		context.arc(0, 0, radius * .475, 0, 2 * Math.PI, false);
		return { handled: false };
	}

	if (shape > 157 && shape < 190) { // Shape Transform
		let borderRadius = (shape - 158) / 31,
			centerAway = radius * (1 - borderRadius),
			scaleRadius = radius * borderRadius;
		context.arc(centerAway, centerAway, scaleRadius, 0, .5 * Math.PI);
		context.arc(-centerAway, centerAway, scaleRadius, .5 * Math.PI, Math.PI);
	...
	// Shape 102+ not handled by drawShapeGeometry - return null to signal fallback needed
	return null;
}

// renderShape - wrapper that handles positioning and calls drawShapeGeometry
// For shapes 102+, uses legacy inline code
function renderShape(context, shape, x, y, radius, stroke = true, fill = true, widthHeightRatio, angle = 0, options = {}) {
	// Try the unified geometry function first (handles shapes < 102, arrays, Path2D, etc)
	context.save();
	context.translate(x, y);
	if (angle) context.rotate(angle);

	const result = drawShapeGeometry(context, shape, radius, { widthHeightRatio, ...options });

	if (result !== null) {
		// drawShapeGeometry handled the shape
	...
	// Restore and handle special shapes 102+ with legacy code (using absolute x,y coordinates)
	context.restore();
	...
	if (shape === 102) {
		for (let [scale, theta] of [
			...
			[1, 1.2 * Math.PI],
			[1, 1.6 * Math.PI]
		]) context.lineTo(x + radius * scale * Math.cos(theta), y + radius * scale * Math.sin(theta));
		...
		for (let i = 0; i < 360; i++) {
			...
			if (i === 135) {
				...
				px = x + radius * Math.cos(((((i + 90) / 360) * 2) * Math.PI));
				py = y + radius * Math.sin(((((i + 90) / 360) * 2) * Math.PI));
			}
			context.lineTo(px, py);
		}
		...
		for (let i = 0; i < 6; i++) {
			let theta = (i + 1) / 6 * 2 * Math.PI,
				htheta = (i + .5) / 6 * 2 * Math.PI,
				c = {
					x: x + radius * dip * Math.cos(htheta),
					y: y + radius * dip * Math.sin(htheta)
				},
				p = {
		...
		for (let i = 0; i < 16; i++) {
			let theta = (i / 16) * 2 * Math.PI,
				x2 = x + radius * Math.cos(theta + .4),
				y2 = y + radius * Math.sin(theta + .4);
			if (i === 1 || i === 5 || i === 9 || i === 13) {
				x2 = x;
				y2 = y;
			}
		...
		for (let i = 0; i < 12; i++) {
			...
			if (i === 2 || i === 10 || i === 6) {
				x2 = x + (radius / 2) * Math.cos(theta);
				y2 = y + (radius / 2) * Math.sin(theta);
			}
			if (i === 0 || i === 4 || i === 8) {
				x2 = x + (radius * 1.25) * Math.cos(theta);
				y2 = y + (radius * 1.25) * Math.sin(theta);
			}
			context.lineTo(x2, y2);
		}
		...
		]) context.lineTo(x + radius * scale * Math.cos((theta - .025)), y + radius * scale * Math.sin((theta - .025)));
		...
		for (let [scale, theta] of [
			...
			[1.1, -0.335],
			[1.5, -0.14]
		]) context.lineTo(x + radius * (scale * 1.5) * Math.cos((theta - .0261)), y + radius * (scale * 1.5) * Math.sin((theta - .0261)));
		...
		for (let [scale, theta] of [
			...
			[-1, .82],
			[0.95, -0.578]
		]) context.lineTo(x + radius * (scale * .9) * Math.cos(theta), y + radius * (scale * .9) * Math.sin(theta));
		...
		for (let [scale, theta] of [
			...
			[-0.35, 1.361],
			[0.625, -0.698]
		]) context.lineTo(x + radius * scale * Math.cos((theta - .0261)), y + radius * scale * Math.sin((theta - .0261)));
		...
		for (let [scale, theta] of [
			...
			[1.315, -0.331613],
			[1, 0]
		]) context.lineTo(x + radius * scale * Math.cos(theta + 1.5447), y + radius * scale * Math.sin(theta + 1.5447));
		...
		context.arc(x, y, radius / 1.15, 0, 2 * Math.PI, false);
		...
		for (let i = 0; i < realShape; i++) {
			let theta = i / realShape * 2 * Math.PI,
				x2 = x + radius * 1.5 * Math.cos(theta + 45),
				y2 = y + radius * 1.5 * Math.sin(theta + 45);
			context.lineTo(x2, y2);
		}
		...
		context.arc(x, y, radius, 0, 2 * Math.PI, true);
		context.arc(x, y, radius / 1.05, 0, 2 * Math.PI, false);

		// Glass Smasher Body
	} else if (shape === 132) {
		context.arc(x, y, radius, 0, 2 * Math.PI, true);
		context.arc(x, y, radius / 1.5, 0, 2 * Math.PI, false);

		// Hendecagon (Rotated)
	} else if (shape === 133) {
		for (let i = 0; i < 11; i++) {
			let theta = i / 11 * 2 * Math.PI,
				x = x + radius * 1.5 * Math.cos((180 / 11) + theta + 1.635),
				y = y + radius * 1.5 * Math.sin((180 / 11) + theta + 1.635);
			context.lineTo(x, y);
		}
		...
		for (let i = 0; i < realShape; i++) {
			let theta = i / realShape * 2 * Math.PI,
				x = x + radius * 1.5 * Math.cos((180 / 4) + theta + .52),
				y = y + radius * 1.5 * Math.sin((180 / 4) + theta + .52);
			context.lineTo(x, y);
		}
		...
		for (let i = 0; i < 6; i++) {
			let theta = i / 6 * 2 * Math.PI,
				x = x + radius * 1.1 * Math.cos((180 / 6) + theta + .385),
				y = y + radius * 1.1 * Math.sin((180 / 6) + theta + .385);
			context.lineTo(x, y);
		}
		...
		context.arc(x, y, radius, 0, 2 * Math.PI, true);
		context.arc(x, y, radius * .999999, 0, 2 * Math.PI, false);
		...
		context.arc(x, y, radius * 1.45, 0, 2 * Math.PI);
		...
		for (let [scale, theta] of [
			...
			[1, 270],
			[1, 288]
		]) context.lineTo(x + radius * scale * Math.cos((theta * Math.PI) / 180), y + radius * scale * Math.sin((theta * Math.PI) / 180));
		...
		for (let i = 0; i < 360; i++) {
			...
			if (i === 180) {
				...
				x2 = x + radius * Math.cos(((((i + 180) / 360) * 2) * Math.PI));
				y2 = y + radius * Math.sin(((((i + 180) / 360) * 2) * Math.PI));
			}
			context.lineTo(x2, y2);
		}
		...
		context.arc(x, y, radius, 0, 2 * Math.PI, true);
		context.arc(x, y, radius * 1.1, 0, 2 * Math.PI, false);
		...
		context.arc(x, y, radius, Math.PI / 2, 3 * Math.PI / 2);
		...
		context.arc(x, y, radius, 0, 2 * Math.PI, true);
		context.arc(x, y, radius * .475, 0, 2 * Math.PI, false);

		// Shape Transform
	} else if (shape > 157 && shape < 190) {
		...
		let path = path2dCache.get(shape) || 2 * Math.PI,
			radiusDiv = 1;
		switch (shape) {
			...
			// Note: angle rotation handled by context.rotate() in Path2D rendering
			...
			case 270: // Hexachoron Rectangle
				...
				radiusDiv = 70;
				// angle rotation is handled by context.rotate(angle) in Path2D section
				break;
		...
		path2dCache.set(shape, { path, radiusDiv })
		radius /= radiusDiv;
		context.save();
		context.translate(x, y);
		...
		context.arc(x, y, radius, 0, Math.PI, 1);
		...
		switch (shape) {
			...
			case 10001:
				...
				break;
			case 10002:
				if (!imageCache.ice_hue || !imageCache.ice_hue.ready) break;
				context.drawImage(imageCache.ice_hue, -radius * 3, -radius * 3, radius * 6, radius * 6);
				break;
			case 10003:
				if (!imageCache.poison_hue || !imageCache.poison_hue.ready) break;
...
function renderProp() {
	...
	return function (ctx, p, pColor, rot, xx, yy, drawSize, m, source) {
		...
		// Calculate position and radius for the prop
		let positionScale = drawSize / m.size * m.realSize;
		let propX = p.x * positionScale + xx;
		let propY = p.y * positionScale + yy;

		if (Array.isArray(p.shape)) {
			// Custom point arrays still need the old method due to bezier control points
			ctx.beginPath();
			...
			let radius;
			if (p.shape === 0) {
				radius = p.scaleSize === true ? drawSize / m.size * m.realSize * p.size : p.size;
			} else if (p.shape > 0) {
				radius = p.scaleSize === true ? (drawSize + p.size) / m.size * m.realSize * p.size : p.size;
			} else {
				radius = p.scaleSize === true ? (drawSize / m.size * m.realSize * p.size) : p.size;
			}

			// Handle aura special case before using renderShape
			if (p.isAura) {
			...
			// Use renderShape for all numeric shapes
			renderShape(ctx, p.shape, propX, propY, radius, !p.borderless, p.fill, null, finalRot, {
				dip: p.shape < 0 ? (p.dip - 6 / (p.shape * p.shape)) : undefined,
				arcLen: p.arclen || 1,
				ring: p.ring
			});

			// Handle special colors after renderShape
			if (p.color >= 1000) {
				ctx.save();
				ctx.clip();
...
function renderPropsAtLayer(context, props, layer, options) {
	const { instance, invulnTicker, renderColor, renderBlend, adjustedRot, tankDrawX, tankDrawY, drawSize, m, source, drawProp } = options;
	const useBlend = layer >= 0;
	...
	for (let i = 0; i < props.length; i++) {
		...
		if (useBlend) pColor = mixColors(pColor, renderColor, renderBlend);
		if (invulnTicker) pColor = mixColors(pColor, color.vlgrey, 0.5);

		setColors(context, pColor);
		if (!p.stroke) context.strokeStyle = context.fillStyle;
		drawProp(context, p, pColor, adjustedRot, tankDrawX, tankDrawY, drawSize, m, source);
	}
}
...
// Cache for static (non-live) entity renders
const staticRenderCache = new Map();
const MAX_STATIC_CACHE_SIZE = 500;

/**
 * Generates a cache key for static entity renders.
 * Includes all properties that affect visual appearance.
 */
function getStaticRenderCacheKey(instance, rot, scale, render) {
	return [
		instance.index,
		instance.color,
		(instance.size * 1000) | 0,
		(rot * 1000) | 0,
		(scale * 1000) | 0,
		render?.status?.getColor?.() || '',
		render?.status?.getBlend?.() || 0,
		instance.widthHeightRatio || 1,
		currentSettings.borderWidth.value.number,
		currentSettings.shaders.value.selected,
	].join('|');
}

function renderEntity(instance, liveRender, options = {}) {
	const {
		rot = 0,
		scale = 1,
		alpha = 1,
	...
	if (currentSettings.hideMiniRenders.value.enabled && !liveRender) {
		return null;
	}
	...
	const gameScale = gameState.screenScale;
	const ratio = gameState.fovScale;
	const drawSize = gameScale * ratio * instance.size * scale;
	const fade = 1;

	// --- CHECK CACHE FOR NON-LIVE RENDERS ---
	if (!liveRender && !turretInfo) {
		const cacheKey = getStaticRenderCacheKey(instance, rot, scale, render);
		const cached = staticRenderCache.get(cacheKey);
	...
	const canvasSize = Math.max(Math.ceil(drawSize * m.position.axis + ratio * 7.5 * instance.size * scale), 1);
	...
	const centerX = canvasSize / 2 - drawSize * 2 * m.position.axis * m.position.middle.x * Math.cos(rot) / 4;
	const centerY = canvasSize / 2 - drawSize * 2 * m.position.axis * m.position.middle.x * Math.sin(rot) / 4;

	// Render entity to offscreen canvas
	renderEntityToContext(
		offscreenCtx, instance, source, m, render,
		centerX, centerY, drawSize, rot, fade,
		gameScale, ratio, turretsObeyRot, alpha,
		// Recursive render function for turrets
		(turretInstance, turretOpts) => renderEntity(turretInstance, liveRender, turretOpts)
	);
	...
	if (!liveRender && !turretInfo) {
		// Convert to ImageBitmap for better performance (if supported)
		// For now, use the canvas directly as it's synchronous
		result = {
			image: canvas,
			centerX,
			centerY,
		...
		// Store in cache
		staticRenderCache.set(getStaticRenderCacheKey(instance, rot, scale, render), result);

		// Prune cache if too large
		if (staticRenderCache.size > MAX_STATIC_CACHE_SIZE) {
			const keysToDelete = [...staticRenderCache.keys()].slice(0, 100);
			keysToDelete.forEach(k => staticRenderCache.delete(k));
		}
	} else {
		// Live render - return canvas directly (not cached)
		result = {
			image: canvas,
			centerX,
			centerY,
	...
	// Clear gun cache if oversized
	if (gunCache.size > 4000) gunCache.clear();

	return result;
}

/**
 * Draws a rendered entity result to a context at specified position.
 *
 * @param {CanvasRenderingContext2D} context - Target context
 * @param {{ image: CanvasImageSource, centerX: number, centerY: number }} renderResult - Result from renderEntity
 * @param {number} x - X position to draw at
 * @param {number} y - Y position to draw at
 * @param {number} [alpha=1] - Transparency
 */
function drawRenderedEntity(context, renderResult, x, y, alpha = 1) {
	...
	context.drawImage(
		renderResult.image,
		Math.round(x - renderResult.centerX),
		Math.round(y - renderResult.centerY)
	);
	context.restore();
}
 ...
 */
function renderEntityToContext(
	currentContext, instance, source, m, render,
	tankDrawX, tankDrawY, drawSize, rot, fade,
	gameScale, ratio, turretsObeyRot, alpha,
	renderEntityRecursive
) {
	// --- CONTEXT SETTINGS ---
	...
	if (render.real) {
		shadowRelativeColor = applyShaderSettings(
			currentContext,
			currentSettings.shaders.value.selected,
			tankDrawX, tankDrawY
		);
	...
	// --- COLOR CALCULATION ---
	source.guns?.update?.();
	const renderColor = render.status.getColor();
	const renderBlend = render.status.getBlend();
	let finalColor = mixColors(getColor(instance.color), renderColor, renderBlend);
	if (fade > 0) finalColor = mixColors(finalColor, color.dgrey, 1 - fade);
	if (instance === global.player.instance) global._tankMenuColor = finalColor;
	const invulnTicker = instance.invuln && (Date.now() - instance.invuln) % 200 > 110;
	if (invulnTicker) finalColor = mixColors(finalColor, color.vlgrey, 0.5);
	...
	if (gameScale < 1) currentContext.lineWidth *= gameScale;

	// --- RENDERING OPTIONS ---
	const props = handleAnimations(instance.id, m.props);
	const renderOptions = {
		instance, source, m, props,
		adjustedRot, tankDrawX, tankDrawY, drawSize,
		renderColor, renderBlend, invulnTicker,
		ratio, alpha, turretsObeyRot, currentContext, render,
		drawProp
	};

	// --- LEASH RENDERING ---
	renderLeash(currentContext, instance, tankDrawX, tankDrawY, drawSize, fade, ratio);

	// --- LAYER -2 PROPS ---
	if (props.length) renderPropsAtLayer(currentContext, props, -2, renderOptions);

	// --- LAYER 0 TURRETS ---
	renderTurretsAtLayerNew(renderOptions, 0, renderEntityRecursive);

	// --- LAYER -1 PROPS ---
	if (props.length) renderPropsAtLayer(currentContext, props, -1, renderOptions);

	// --- GUNS ---
	renderGuns(currentContext, renderOptions);

	// --- LAYER 0 PROPS ---
	if (props.length) renderPropsAtLayer(currentContext, props, 0, renderOptions);

	// --- BODY ---
	setColors(currentContext, finalColor);
	if (shadowRelativeColor) currentContext.shadowColor = currentContext.strokeStyle;

	const bodyRadius = drawSize / m.size * m.realSize;

	if (instance.color >= 1000) {
		// Special color - clip and apply
		currentContext.save();
		currentContext.beginPath();
		renderShape(currentContext, m.shape, tankDrawX, tankDrawY, bodyRadius, false, false, source.widthHeightRatio, adjustedRot);
		currentContext.clip();
		specialColors[instance.color](currentContext, instance);
		currentContext.restore();
	} else {
		// Normal body render
		renderShape(currentContext, m.shape, tankDrawX, tankDrawY, bodyRadius, true, true, source.widthHeightRatio, adjustedRot);
	}
	...
	// --- LAYER 1 TURRETS ---
	renderTurretsAtLayerNew(renderOptions, 1, renderEntityRecursive);
...
/**
 * Renders turrets at a specific layer using the new renderEntity signature.
 */
function renderTurretsAtLayerNew(options, layer, renderEntityRecursive) {
	const { source, m, adjustedRot, drawSize, tankDrawX, tankDrawY, ratio, alpha, turretsObeyRot, currentContext, render } = options;

	if (source.turrets.length !== m.turrets.length) return;
	...
	for (let i = 0; i < m.turrets.length; i++) {
		...
		const ang = t.direction + t.angle + adjustedRot;
		...
		const turretX = tankDrawX + len * Math.cos(ang);
		const turretY = tankDrawY + len * Math.sin(ang);

		// Render turret and draw it
		const turretResult = renderEntityRecursive(t, {
			rot: (source.turrets?.[i]?.lerpedFacing || 0) + turretsObeyRot * adjustedRot,
			scale: drawSize / ratio / t.size * t.sizeFactor,
			alpha,
			turretsObeyRot,
			turretInfo: source.turrets[i],
			render,
		});

		if (turretResult) {
			drawRenderedEntity(currentContext, turretResult, turretX, turretY, 1);
		}
	}
}

export { renderEntity, drawRenderedEntity }