...
function drawShape(context, shape, size, stroke, fill, options = {}) {
	...
	if (shape instanceof Path2D) {
		let sizeDiv = shape.path2dDiv || 1;
		let scaledSize = size / sizeDiv;
		context.save();
	...
	if (Array.isArray(shape)) {
		// SVG path string format: [pathString, sizeDiv]
		if (!shape[2] && typeof shape[0] === 'string') {
			let sizeDiv = shape[1] || 1;
		...
		for (let [sx, sy] of shape) {
			context.lineTo(size * sx, size * sy);
		}
		context.closePath();
	...
	if (shape > 0 && shape < 102) {
		if (shape === 4 && widthHeightRatio != null && (widthHeightRatio[0] !== 1 && widthHeightRatio[1] !== 1)) {
			...
			for (let i = 0; i < sides; i++) {
				let [rx, ry] = allPoints[i];
				context.lineTo(size * rx + (rx - 1) * 1.1, size + (ry - 1) * 1.1);
			}
		} else {
			let sides = Math.ceil(shape * arcLen);
			for (let i = 0; i < sides; i++) {
	...
	if (shape === 104) { // Star
		...
		for (let i = 0; i < 6; i++) {
			let theta = (i + 1) / 6 * 2 * Math.PI,
				...
				cx = size * dip * Math.cos(htheta),
				cy = size * dip * Math.sin(htheta),
				px = size * Math.cos(theta),
				py = size * Math.sin(theta);
	...
	if (shape === 105) { // Nautica shell shape
		for (let i = 0; i < 16; i++) {
			let theta = (i / 16) * 2 * Math.PI,
				px = size * Math.cos(theta + .4),
				py = size * Math.sin(theta + .4);
			if (i === 1 || i === 5 || i === 9 || i === 13) {
				px = 0;
				py = 0;
			}
	...
	if (shape === 108) { // Tri-Blade
		for (let i = 0; i < 12; i++) {
			let theta = (i / 12) * 2 * Math.PI,
				...
				py = size * Math.sin(theta);
			if (i === 2 || i === 10 || i === 6) {
				px = (size / 2) * Math.cos(theta);
				py = (size / 2) * Math.sin(theta);
			}
			if (i === 0 || i === 4 || i === 8) {
				px = (size * 1.25) * Math.cos(theta);
				py = (size * 1.25) * Math.sin(theta);
			}
			context.lineTo(px, py);
		}
		if (fill) context.fill();
	...
	if (shape === 125) { // Caravan
		let bordersize = 0.4,
			centerAway = size * (1 - bordersize),
			scalesize = size * bordersize;
		context.arc(centerAway, centerAway, scalesize, 0, .5 * Math.PI);
	...
	if (shape === 133) { // Hendecagon (Rotated)
		for (let i = 0; i < 11; i++) {
			let theta = i / 11 * 2 * Math.PI,
				px = size * 1.5 * Math.cos((180 / 11) + theta + 1.635),
	...
	if (shape === 134) { // Square (Rotated)
		...
		for (let i = 0; i < realShape; i++) {
			let theta = i / realShape * 2 * Math.PI,
				px = size * 1.5 * Math.cos((180 / 4) + theta + .52),
	...
	if (shape === 135) { // Hexagon (Rotated)
		for (let i = 0; i < 6; i++) {
			let theta = i / 6 * 2 * Math.PI,
				px = size * 1.1 * Math.cos((180 / 6) + theta + .385),
	...
	if (shape === 145) { // Aquamarine Body Type
		for (let i = 0; i < 360; i++) {
			...
			if (i === 180) {
				...
				px = size * Math.cos(((((i + 180) / 360) * 2) * Math.PI));
				py = size * Math.sin(((((i + 180) / 360) * 2) * Math.PI));
			}
			context.lineTo(px, py);
		}
		if (fill) context.fill();
	...
	if (shape === 157) { // Injector
		context.arc(0, 0, size, 0, 2 * Math.PI, true);
		context.arc(0, 0, size * .475, 0, 2 * Math.PI, false);
		if (fill) context.fill();
		if (stroke) context.stroke();
		return true;
	}
	...
	if (shape > 9999) {
		...
		switch (shape) {
			case 10000:
				...
				context.drawImage(imageCache.ned, -size * 1.5, -size * 2.5, size * 3, size * 5);
				return true;
			case 10001:
				if (!imageCache.do_not_open_at_any_cost || !imageCache.do_not_open_at_any_cost.ready) break;
				context.drawImage(imageCache.do_not_open_at_any_cost, -size, -size, size * 2, size * 2);
				return true;
			case 10002:
				if (!imageCache.ice_hue || !imageCache.ice_hue.ready) break;
				context.drawImage(imageCache.ice_hue, -size * 3, -size * 3, size * 6, size * 6);
				return true;
			case 10003:
				if (!imageCache.poison_hue || !imageCache.poison_hue.ready) break;
				context.drawImage(imageCache.poison_hue, -size * 3, -size * 3, size * 6, size * 6);
				return true;
			case 10004:
				if (!imageCache.ied || !imageCache.ied.ready) break;
				context.drawImage(imageCache.ied, -size, -size, size * 2, size * 2);
				return true;
			case 10005:
				if (!imageCache.danksGun || !imageCache.danksGun.ready) break;
				context.drawImage(imageCache.danksGun, -size, -size, size * 2.25, size * 2);
				return true;
			case 10006:
				if (!imageCache.hotWheels || !imageCache.hotWheels.ready) break;
				context.drawImage(imageCache.hotWheels, -size, -size, size * 2, size * 2);
				return true;
			case 10007:
				if (!imageCache.omega || !imageCache.omega.ready) break;
				context.drawImage(imageCache.omega, -size, -size, size * 2, size * 2);
				return true;
			case 10008:
				if (!imageCache.missingno || !imageCache.missingno.ready) break;
				context.drawImage(imageCache.missingno, -size, -size * 1.5, size * 2, size * 3);
				return true;
			case 10009:
				if (!imageCache.speedy || !imageCache.speedy.ready) break;
				context.drawImage(imageCache.speedy, -size, -size, size * 2, size * 2);
				return true;
			case 10010:
				if (!imageCache.marble_swirl || !imageCache.marble_swirl.ready) break;
				context.drawImage(imageCache.marble_swirl, -size, -size, size * 2, size * 2);
				return true;
			case 10011:
				if (!imageCache.fourFour || !imageCache.fourFour.ready) break;
				context.drawImage(imageCache.fourFour, -size, -size * 1.5, size * 2, size * 3);
				return true;
			case 10012:
				if (!imageCache.emp_hue || !imageCache.emp_hue.ready) break;
				context.drawImage(imageCache.emp_hue, -size * 2, -size * 2, size * 4, size * 4);
				return true;
			case 10013:
				if (!imageCache.par_hue || !imageCache.par_hue.ready) break;
				context.drawImage(imageCache.par_hue, -size * 2, -size * 2, size * 4, size * 4);
				return true;
			case 10014:
				if (!imageCache.soccerballs || !imageCache.soccerballs.ready) break;
				context.drawImage(imageCache.soccerballs, -size, -size, size * 2, size * 2);
				return true;
			case 10015:
				if (!imageCache.boyKisser || !imageCache.boyKisser.ready) break;
				context.drawImage(imageCache.boyKisser, -size * 5, -size * 5, size * 10, size * 10);
				return true;
			case 10016:
				if (!imageCache.tonk || !imageCache.tonk.ready) break;
				context.drawImage(imageCache.tonk, -size * 2, -size * 2, size * 4, size * 4);
				return true;
			case 10017:
				context.rotate(-angle);
				if (!imageCache.rollfac || !imageCache.rollfac.ready) break;
				context.drawImage(imageCache.rollfac, -size, -size, size * 2, size * 2);
				context.rotate(angle);
				return true;
			case 10018:
				...
				// profit
				context.drawImage(image, -size * 1.75, -size, size * 3.5, size * 2);
				return true;
			case 10019:
				if (!imageCache.treadmarks || !imageCache.treadmarks.ready) break;
				context.drawImage(imageCache.treadmarks, -size, -size, size * 2, size * 2);
				return true;
			case 10020: // This is not a mistake, leave this blank
				return true;
			case 10021:
				if (!imageCache.seniorpentagon || !imageCache.seniorpentagon.ready) break;
				context.drawImage(imageCache.seniorpentagon, -size, -size, size * 2, size * 2);
				return true;
		}
	}
	return false;
...
function renderProp(ctx, entity, prop, propColor) {
	...
	let propX = prop.x;
	let propY = prop.y;
	let propSize = prop.size;
	let propRot = prop.angle;
	if (prop.rpm) propRot += rpmAngle
	if (prop.lockRot === false) propRot += entity.angle;
	...
	if (prop.isAura) {
		let grad = getGradient(propColor);
	...
	if (Array.isArray(prop.shape)) {
		// Custom point arrays with bezier control points
		// Points are defined as [x, y] which get converted to polar (scale, angle)
		// then back to Cartesian with prop position offset added
		ctx.beginPath();
		for (let [x, y, cx1, cy1, cx2, cy2] of prop.shape) {
			// Convert point to polar then back to Cartesian (matching old pointInit behavior)
			...
			if (cx2 !== undefined) {
				// Bezier curve - control points are absolute coordinates
				let c1Scale = Math.hypot(cx1, cy1);
				let c1Angle = Math.atan2(cx1, cy1);
				let c2Scale = Math.hypot(cx2, cy2);
				let c2Angle = Math.atan2(cx2, cy2);
				ctx.bezierCurveTo(
				...
				// Quadratic curve - control point goes through same transform
				let c1Scale = Math.hypot(cx1, cy1);
				let c1Angle = Math.atan2(cx1, cy1);
	...
	if (prop.color >= 1000) {
		ctx.save();
		ctx.clip();
		specialColors[prop.color](ctx, entity);
		ctx.restore();
	}
...
function handlePropAnimations(entity) {
	...
	if (animations) {
		for (let animation of animations) {
			...
			if (!prop) {
				console.warn("Got animation index for non-existant prop:", entity, animation.index)
				return entity.props;
			}
