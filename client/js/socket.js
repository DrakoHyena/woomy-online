...
function parseMockup(seenIndexes) {
    ...
    let mockup = new ClientEntity(
        ...
        "#FFFFFF", // nameColor
        label, // label
        [1, 1], // widthHeightRatio (array with 2 elements)
        false, // hideHealth
        false, // hideName
        false // leash
    );
    ...
    for (let i = 0; i < gunCount; i++) {
        const offset = convert.reader.next();
        const direction = convert.reader.next();
        const length = convert.reader.next();
        const width = convert.reader.next();
        const aspect = convert.reader.next();
        const angle = convert.reader.next();
        const skin = convert.reader.next();
        const color = convert.reader.next();

        mockup.guns.push(new ClientGun(
            skin,
            color,
            aspect,
            direction,
...
class ClientEntity {
    constructor(
        ...
        if (typeof this.leash === "object") {
            ...
            for (let i = 0; i < 10; i++) {
                this.leash.points.push(
                    new RopePoint(
                        (this.x + this.leash.x) / 2,
                        (this.y + this.leash.y) / 2
                    )
                );
            }
        }
    ...
    tick() {
        ...
        for (let goal in this.goals) {
            switch (goal) {
                // Lerp angle
                case "facing":
                    this[goal] = lerpAngle(this[goal], this.goals[goal], 0.4);
                    break;
                // Lerp rounded
                case "score":
                    this[goal] = Math.round(
                        lerp(this[goal], this.goals[goal], 0.4)
                    );
                    break;
                // Lerp
                default:
                    this[goal] = lerp(this[goal], this.goals[goal], 0.4);
                    break;
            }
            if (goal === "facing") {
            } else {
...
function newEntity(id, skipSpawnFade = false) {
    let entity = new ClientEntity(
        ...
        convert.reader.next()
            ? { x: convert.reader.next(), y: convert.reader.next() }
            : false // leash
    );
    ...
    if (updateType >= 1) {
        if (convert.reader.next()) {
            ...
            if (!entity.leash) {
                ...
                for (let i = 0; i < 10; i++) {
                    entity.leash.points.push(
                        new RopePoint(
                            (entity.x + entity.leash.x) / 2,
                            (entity.y + entity.leash.y) / 2
                        )
                    );
                }
            } else {
...
function convertEntities() {
    for (
        let i = 0, incomingEntityAmount = convert.reader.next();
        i < incomingEntityAmount;
        i++
    ) {
        const id = convert.reader.next();
...
function convertFastGui() {
    playerState.levelProgress = convert.reader.next() * 0.1; // multiplied by 10 before sending
    ...
    if (skillStatChanges) {
        for (let i = 0; i < skillStatChanges; i++) {
            ...
            playerState.gui.skills[skillName] = {
                max: convert.reader.next(),
                current: convert.reader.next()
            };
        }
    }
}
...
async function onmessage(message) {
    ...
    switch (packet) {
        ...
        case serverPackets.layerInfo:
            ...
            roomState.cellSkins = Object.assign(
                roomState.cellSkins,
                JSON.parse(m[i++])
            );
        ...
        case serverPackets.gameMessage:
            ...
            roomState.chatMessages.push(
                new ChatMessage(m[0], m[1], m[2], m[3])
            );
            break;
        ...
        case serverPackets.viewUpdate:
            ...
            playerState.camera.x = lerp(
                playerState.camera.x,
                cam.x,
                window.movementSmoothing * 0.7
            );
            playerState.camera.y = lerp(
                playerState.camera.y,
                cam.y,
                window.movementSmoothing * 0.7
            );
            playerState.camera.fov = lerp(
                playerState.camera.fov,
                cam.FoV,
                window.movementSmoothing * 0.7
            );
            ...
            socket.send(
                clientPackets.inputUpdate,
                ...gameState.lastInput.changes
            );
...
let connectClientSocket = async function(roomId) {
    ...
    socket.send(
        "k",
        currentSettings.networkProtocolVersion.value.number,
